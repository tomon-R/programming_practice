...
int main() {
    ...
    std::string kFile = (outputDir / "K.txt").string();
    std::string bFile = (outputDir / "b.txt").string();
    std::string rhsFile = (outputDir / "rhs.txt").string();
    std::string fFile = (outputDir / "f.txt").string();
    std::string uFile = (outputDir / "u.txt").string();
    ...
    std::string theoreticalFile = (outputDir / "theoretical.vtk").string();
    ...
    std::string errFile = (outputDir / "err.vtk").string();
    ...
    for (const auto& element : elements) {
        ...
        if (element.type == 5) {
            ...
            if (interpolationMethod == "linear") {
                ...
                Ke[0][0] = D / 2.0 * (b0 * b0 + c0 * c0);
                Ke[0][1] = D / 2.0 * (b0 * b1 + c0 * c1);
                Ke[0][2] = D / 2.0 * (b0 * b2 + c0 * c2);
                Ke[1][0] = D / 2.0 * (b1 * b0 + c1 * c0);
                Ke[1][1] = D / 2.0 * (b1 * b1 + c1 * c1);
                Ke[1][2] = D / 2.0 * (b1 * b2 + c1 * c2);
                Ke[2][0] = D / 2.0 * (b2 * b0 + c2 * c0);
                Ke[2][1] = D / 2.0 * (b2 * b1 + c2 * c1);
                Ke[2][2] = D / 2.0 * (b2 * b2 + c2 * c2);

                fs::path outputDir = OUTPUT_DIR;
                std::string KeFile = (outputDir / "Ke.txt").string();
    ...
    for (const auto& element : elements) {
        ...
        if (element.type == 5) {
            ...
            if (interpolationMethod == "linear") {
                ...
                fs::path outputDir = OUTPUT_DIR;
                std::string feFile = (outputDir / "fe.txt").string();
